...
<a class="btn btn-success btn-outline-success" href="{{ route('logout') }}"  onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
  ...
  <div class="content-wrapper">
      <section class="content-header">
          <h1>
              {{$MORAT==null?"AJOUT D'UN MORATOIRE":"MODIFICATION DU MORATOIRE".$MORAT->name}}
            <small></small>
          </h1>

        </section>
      <form role="form" method="POST" action="{{ $MORAT==null?route('moratoires.store'):route('moratoires.update',$MORAT)}}" enctype="multipart/form-data">
            @if($MORAT!=null) @method('PATCH') @endif
            ...
            <div class="box-body">
                ...
                <div class="form-group" >
                    ...
                    <input type="text" name="name" class="form-control  @error('name') is-invalid @enderror" id="adminnom" placeholder="Entrer un nom" value="{{ $MORAT==null?old('name'):$MORAT->name }}"  autocomplete="name" autofocus required>
                ...
                <div class="form-group" >
                    <label for="delais">date d'expiration</label>
                    <input type="date" name="delais" class="form-control  @error('delais') is-invalid @enderror" id="delais" placeholder="date d'expiration" value="{{ $MORAT==null?old('delais'):$MORAT->delais}}"  autocomplete="delais" autofocus required>
                  ...
                  @if(count($ETUDS)!=0)
                <div class="form-group">
                    <label for="Inputfiliere"> l'etudiant</label>
                    <select class="form-control" id="Inputfiliere" name="etudiant">
                            @foreach($ETUDS as $ETUD)
                        <option value="{{$ETUD->id}}">{{$ETUD->name}}</option>
                             @endforeach
                    </select>
                    @error('filiere')
                    <span class="invalid-feedback" role="alert">
                 ...
                 @else
                      <span class="label label-danger">Il n'ya pas d'etudiant </span>
                @endif
                ...
                <div class="form-group" >
                    ...
                    <textarea name="description" class="form-control  @error('description') is-invalid @enderror" id="desc" placeholder="Entrer une description" value="{{ $MORAT==null?old('description'):$MORAT->description }}"  autocomplete="description" autofocus>{{ $MORAT==null?old('description'):$MORAT->description }}</textarea>
                ...
                @if(count($ETUDS)!=0 || count($UNITS)!=0)
                <div class="box-footer">
              <button type="submit" class="btn btn-primary">{{$MORAT==null?"AJOUTER":"MODIFIER"}}</button>
                </div>
                    @endif
            </div>
          </form>
