...
<head>
 ...
 <title>{{ config('app.name', 'INSCRIPTION_IAI') }}</title>
  ...
  <!-- ETUDLTE Skins. Choose a skin from the css/skins
        ...
        <div class="row">
            <div class="col-md-4">
              ...
              <div class="box box-primary">
                <div class="box-body box-profile">
                  ...
                  <h3 class="profile-username text-center">{{$ETUD->name.'  '.$ETUD->surname}}</h3>

                  <ul class="list-group list-group-unbordered">
                    <li class="list-group-item">
                      <b>contact</b> <a class="pull-right">{{$ETUD->phone}}</a>
                    </li>
                      <li class="list-group-item">
                          <b>residence</b> <a class="pull-right">{{$ETUD->residence}}</a>
                      </li>
                    <li class="list-group-item">
                      <b>adrese email</b> <a class="pull-right">{{$ETUD->email}}</a>
                    </li>
                    <li class="list-group-item">
                        <p>
                            {{$ETUD->description}}
                  ...
                  <a href="{{route('etudiants.edit',[$ETUD])}}" class="btn btn-primary btn-block"><i class="fa fa-pencil"></i> <b>Modifier ce profil</b></a>

                  @if($ETUD->id != Auth::user()->id)
                   ...
                   @endif
                    <a href="{{route('etudiants.create')}}" class="btn btn-success btn-block" >  <i class="fa fa-user-plus"></i> <b>nouvel ETUDIANT</b></a>
                  <form action="{{route('etudiants.destroy',[$ETUD])}}" method="post" id="delete-form">
                      @csrf
                      @method('DELETE')
                  </form>
            ...
            <div class="col-md-8">
                <div class="box box-primary">
                    ...
                    <!-- /.box-header -->
                    <div class="box-body">
                        <strong><i class="fa fa-money margin-r-5"></i> Versement</strong>

                        <p class="text-muted">
                            TOTAL(FCFA): {{$ETUD->VERSEMENT()}}<br>
                            RESTE(FCFA): {{$ETUD->VERSEMENT_RESTE()}}<br>
                            EVOLUTION: {{$ETUD->VERSEMENT_EVOLUTION()}}<br>
                            <b>status:</b>
                            @if(!$ETUD->moratoire)
                            @if($ETUD->insolvable)<span class="label label-danger"> insolvable</span>
                            @else<span class="label label-success"> solvable</span>
                            @endif
                            @else <span class="label label-warning"> moratoire</span>@endif
                        </p>

                        <hr>

                        <strong><i class="fa fa-money margin-r-5"></i> extras</strong>

                        <p class="text-muted">
                            PENALITE (FCFA): {{$ETUD->montantpenalite}}<br>
                          @if($ETUD->MORATOIRE()!=null)  MORATOIRE (FCFA): {{$ETUD->MORATOIRE()->MONTANT()}}@endif
                        </p>

                        <hr>

                        <strong><i class="fa fa-square  margin-r-5"></i> Operations</strong>
                        <a href="{{route('paiements.create',$ETUD)}}" class="btn btn-primary btn-sm"><b>completer paiement</b></a>
                        <a href="{{route('moratoires.create',$ETUD)}}" class="btn btn-primary btn-sm"><b>nouveau moratoire</b></a>
                        <a href="{{route('penalites.erase',$ETUD)}}" class="btn btn-primary btn-sm"><b>annuler les penalite</b></a>
                        <a href="{{route('moratoires.erase',$ETUD)}}" class="btn btn-primary btn-sm"><b>annuler le moratoire</b></a>
...
<!-- ETUDLTE App -->
<script src="{{ asset('/public/dist/js/ETUDlte.min.js') }}"></script>
<!-- ETUDLTE dashboard demo (This is only for demo purposes) -->
<script src="{{ asset('/public/dist/js/pages/dashboard.js') }}"></script>
<!-- ChartJS -->
<script src="{{ asset('/public/bower_components/chart.js/Chart.js') }}"></script>
<!-- ETUDLTE for demo purposes -->
