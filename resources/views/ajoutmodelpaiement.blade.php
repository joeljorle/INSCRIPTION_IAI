...
<head>
 ...
 <title>{{ config('app.name', 'INSCRIPTION_IAI') }}</title>
  ...
  <!-- WARNING: Respond.js doesn't work if you view the page via model:// -->
  <!--[if lt IE 9]>
      ...
      <ul class="sidebar-menu" data-widget="tree">
          ...
          <li class="active treeview menu-open">
            ...
            <ul class="treeview-menu">
              ...
              <li><a href="{{route('modelpaiements.index')}}"><i class="fa fa-institution"></i>modelpaiements</a></li>
...
<li><a href="{{route('modelpaiements.index')}}"><i class="fa fa-square-o"></i>models de paiement</a></li>
  ...
  <div class="content-wrapper">
      <section class="content-header">
          <h1>
              {{$MODEL==null?"AJOUT D'UN MODEL DE PAIEMENT":"MODIFICATION DU MODEL DE PAIEMENT ".$MODEL->name}}
            <small></small>
          </h1>

        </section>
      <form role="form" method="POST" action="{{ $MODEL==null?route('modelpaiements.store'):route('modelpaiements.update',$MODEL)}}" enctype="multipart/form-data">
            @if($MODEL!=null) @method('PATCH') @endif
            ...
            <div class="box-body">
                ...
                <div class="form-group" >
                    ...
                    <input type="text" name="name" class="form-control  @error('name') is-invalid @enderror" id="adminnom" placeholder="Entrer un nom" value="{{ $MODEL==null?old('name'):$MODEL->name }}"  autocomplete="name" autofocus >
                ...
                @if(count($FILS)!=0)
                    <div class="form-group">
                        <label for="Inputfiliere"> sa Filiere</label>
                ...
                @else
                    <span class="label label-danger">Il n'ya pas de filieres </span>
                @endif

                @if(count($UNITS)!=0)
                    <div class="form-group">
                        <label for="Inputunite"> son unite de paiement</label>
                        <select class="form-control" id="Inputunite" name="unite">
                ...
                <div class="form-group" >
                    ...
                    <input type="number" min=0 name="montant" class="form-control  @error('montant') is-invalid @enderror" id="adminnom" placeholder="Entrer un montant" value="{{ $MODEL==null?old('montant'):$MODEL->montant }}"  autocomplete="montant" autofocus required >
                ...
                <div class="form-group" >
                    ...
                    <input type="date"  name="delais" class="form-control  @error('delais') is-invalid @enderror" id="delais" placeholder="Choisir ou Entrer un delais" value="{{ $MODEL==null?old('delais'):$MODEL->delais }}"  autocomplete="delais" autofocus required >
                ...
                <div class="form-group" >
                    ...
                    <textarea name="description" class="form-control  @error('description') is-invalid @enderror" id="desc" placeholder="Entrer une description" value="{{ $MODEL==null?old('description'):$MODEL->description }}"  autocomplete="description" autofocus>{{ $MODEL==null?old('description'):$MODEL->description }}</textarea>
                ...
                @if(count($UNITS)!=0 && count($FILS)!=0)
                <div class="box-footer">
              <button type="submit" class="btn btn-primary">{{$MODEL==null?"AJOUTER":"MODIFIER"}}</button>
                </div>
                @endif
            </div>
          </form>
