...
<a class="btn btn-success btn-outline-success" href="{{ route('logout') }}"  onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
  ...
  <div class="content-wrapper">
      <section class="content-header">
          <h1>
              {{$PDIST==null?"AJOUT D'UN PAIEMENT DISTANT":"MODIFIFCATION DU PAIEMENT DISTANT".$PDIST->name}}
            <small></small>
          </h1>

        </section>
      <form role="form" method="POST" action="{{ $PDIST==null?route('paiementdistances.store'):route('paiementdistances.update',$PDIST)}}" enctype="multipart/form-data">
            @if($PDIST!=null) @method('PATCH') @endif
            ...
            <div class="box-body">
                ...
                <div class="form-group" >
                    ...
                    <input type="text" name="name" class="form-control  @error('name') is-invalid @enderror" id="adminnom" placeholder="Entrer un nom" value="{{ $PDIST==null?old('name'):$PDIST->name }}"  autocomplete="name" autofocus required>
                ...
                <div class="form-group" >
                    ...
                    <textarea name="description" class="form-control  @error('description') is-invalid @enderror" id="desc" placeholder="Entrer une description" value="{{ $PDIST==null?old('description'):$PDIST->description }}"  autocomplete="description" autofocus>{{ $PDIST==null?old('description'):$PDIST->description }}</textarea>
              ...
              <button type="submit" class="btn btn-primary">{{$PDIST==null?"AJOUTER":"MODIFIER"}}</button>
                </div>
            </div>
          </form>
