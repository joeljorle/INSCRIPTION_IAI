  ...
  <div class="content-wrapper">
      ...
      @if(count($ETUDS)!=0)
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">rechercher un etudiant</h3>
          </div>
          <!-- /.box-header -->
          <div class="box-body">
        ...
        <form role="form" method="get" action="{{ route('paiement2.store')}}" enctype="multipart/form-data">
            ...
            @csrf
              ...
              <div class="form-group">
                <label>choisir dabord un etudiant</label>


                <small> par le matricule</small>

                  <select class="form-control"  name="etudiant">
                      @foreach($ETUDS as $ETUD)
                          <option value="{{$ETUD->id}}">{{$ETUD->name.'  '.$ETUD->surname.':  '.$ETUD->slug}}</option>
                      @endforeach
                  </select>
                  @error('etudiant')
             ...
             <select class="form-control select2" style="width: 100%;" name="mode">
               <option selected="selected" value="0">local</option>
                 @foreach($PDISTS as $PDIST)
                     <option value="{{$PDIST->id}}">{{$PDIST->name}}</option>
                 @endforeach
             </select>
            </div>
      ...
      @else
          ...
          <div class="box box-danger bg-red-gradient">
              <div class="box-header">
                  <label> IL n'y a pas d'etudiant</label>
              </div>
              <div class="box-body align-items-center">
                  <a class="center-block btn btn-outline-success" href="{{route('etudiants.create')}}"> AJOUTER UN ETUDIANT</a>
              </div>
